...
const App: React.FC = () => {
  ...
  const priceGroups = useMemo(() => getPriceGroups(filteredFlights), [filteredFlights]);
    ...
    <div className="min-h-screen bg-surface-50 text-white font-sans selection:bg-primary-500/30">
          ...
          <div className="hidden md:flex items-center gap-10">
            <nav className="flex items-center gap-8">
              {['Explore', 'My Bookings', 'Elite Club'].map((item) => (
                <a key={item} href="#" className="text-xs font-black uppercase tracking-[0.2em] text-gray-500 hover:text-white transition-colors">{item}</a>
              ))}
            </nav>
      ...
      <main className="container mx-auto px-6 py-12 relative">
        ...
        <div className="mb-16 text-center max-w-3xl mx-auto">
          <div className="inline-block px-4 py-1.5 mb-6 bg-primary-600/10 border border-primary-500/20 rounded-full">
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary-400">Next Gen Travel Platform</span>
          </div>
          <h2 className="text-6xl font-black tracking-tighter mb-6 leading-[0.9]">
            The World is <span className="bg-clip-text text-transparent bg-gradient-to-r from-primary-400 to-accent-blue">Waiting</span> for You.
          </h2>
          <p className="text-gray-400 font-medium text-lg">Experience luxury travel planning with the most advanced flight intelligence engine.</p>
        </div>
        ...
        <div className="relative mb-20">
          ...
          <SearchForm 
            ...
            isLoading={isLoading}
            initialValues={{
              origin: urlState.origin ? { iataCode: urlState.origin, name: urlState.origin, cityName: '', countryName: '' } : null,
              destination: urlState.dest ? { iataCode: urlState.dest, name: urlState.dest, cityName: '', countryName: '' } : null,
              departureDate: urlState.depDate,
        ...
        {error && (
          <div className="glass-card border-red-500/20 bg-red-500/5 p-8 rounded-3xl mb-12 flex items-center gap-6 animate-float">
            ...
            <div>
              <h3 className="text-xl font-black text-white mb-1">System Notice</h3>
          ...
          <div className={flights.length > 0 ? 'lg:col-span-3' : 'lg:col-span-4'}>
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-32">
                ...
                <h3 className="text-2xl font-black tracking-tight mb-2">Analyzing Routes...</h3>
                <p className="text-gray-500 uppercase text-[10px] font-black tracking-[0.3em]">Searching 500+ Luxury Airlines</p>
              </div>
            ) : flights.length > 0 ? (
              <div className="space-y-12">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between mb-8 px-4">
                    <h3 className="text-2xl font-black tracking-tighter">Available <span className="text-primary-500">Curated</span> Options</h3>
                  ...
                  {filteredFlights.length === 0 && (
                    <div className="text-center py-20 glass-card rounded-3xl border-dashed border-white/10">
                      <p className="text-gray-500 font-bold">No flights match your elite criteria.</p>
                      <button onClick={() => setFilters({ ...filters, maxPrice: 2000, airlines: [], stops: [], departureTime: 'all' })} className="mt-4 text-primary-500 font-black uppercase text-[10px] tracking-widest hover:text-white transition-colors">Clear all filters</button>
                    </div>
                  )}
                </div>
              </div>
              ...
              !error && (
                <div className="flex flex-col items-center justify-center py-32 rounded-[3rem] border border-dashed border-white/5 bg-white/2">
                   ...
                   <h3 className="text-2xl font-black mb-2">Begin Your Journey</h3>
                   <p className="text-gray-500 text-sm">Select your origin and destination to discover exclusive routes.</p>
                </div>
              )
            )}
          </div>
